/* AUTO-GENERATED from tasmee-engine.ts — do not edit directly. Regenerate: node scripts/transpile-tasmee.cjs */
"use strict";
...
class TasmeeEngine {
    ...
    async startSession(ayahDataArray, { hideText = false, ayahOffset = 0 } = {}) {
        ...
        if (!SpeechRec)
            throw new Error('المتصفح لا يدعم التعرف على الكلام. استخدم Chrome أو Edge.');
        this._wordTokens = this._buildWordIndex(ayahDataArray);
        if (this._wordTokens.length === 0)
            throw new Error('لا توجد كلمات للتسميع في هذه الصفحة.');
        this._expectedIdx = 0;
        this._ended = false;
        this.isPaused = false;
        this.isActive = true;
    ...
    _matchMergedAt(spokenNorm, idx) {
        if (idx + 1 >= this._wordTokens.length)
            return false;
        ...
        if (lastA === 'ن' && 'ينمولر'.includes(firstB)) {
            ...
            if (spokenNorm === merged || this._fuzzyMatch(spokenNorm, merged))
                return true;
        }
        return false;
    }
    _injectWordSpans(hideText) {
        ...
        this._wordTokens.forEach(t => {
            if (!tokensByAyah[t.ayahIdx])
                tokensByAyah[t.ayahIdx] = [];
        ...
        ayahTexts.forEach((el, domIdx) => {
            ...
            if (ayahIdx >= 0 && tokensByAyah[ayahIdx] && tokensByAyah[ayahIdx].length > 0) {
                this._snapshots.push({ el: el, html: el.innerHTML });
            }
    ...
    _highlightWord(idx, state) {
        if (idx < 0 || idx >= this._wordTokens.length)
            return;
        ...
        if (state !== 'pending')
            span.classList.add(`q-word--${state}`);
        if (state === 'correct' || state === 'fuzzy')
            span.classList.remove('q-word--hidden');
    }
    ...
    _dryRunMatch(transcript) {
        ...
        for (const word of spoken) {
            if (idx >= this._wordTokens.length)
                break;
    ...
    _showResultModal(summary) {
        ...
        if (typeof window.openModal === 'function')
            window.openModal('tasmeeResultsModal');
    }
    _getAudioCtx() {
        try {
    ...
    _playTone(freq, duration) {
        if (!this._options.audioFeedback)
            return;
