// ════════════════════════════════════════════════════════════════════
//  Tasmee' Pro v3 — text-rendered mushaf page (quran_v2.html)
//
//  v3 = v2's rendering/recording layer + two upgrades:
//    1. The matching brain is the shared TasmeeMatcher
//       (js/tasmee-matcher.js): tajweed variants (iqlab/idgham),
//       Levenshtein-DP fuzzy matching, idgham two-word fusion, lookahead
//       skip detection and extra-word tracking — replacing the old inline
//       prefix/1-edit matcher. Falls back to the v2 inline matcher if the
//       shared module failed to load.
//    2. Sessions persist: word states accumulate into a last-wins map and
//       are saved via TasmeeStore (sessions + typed mistakes) on close,
//       so the Tasmee dashboard finally sees Pro-panel usage.
//
...
const _v2 = {
  ...
  fitRaf: null,
  // v3: shared-matcher + session-persistence state
  matcher: null,     // TasmeeMatcher over allTokens.slice(ptr)
  allTokens: null,   // full-page tokens, 1:1 with words[]
  tokenOffset: 0,    // words[] index of matcher token 0
  sessionStart: 0,
  results: null,     // Map "s:a:w" -> {surah, ayah, word, state} (last wins)
  extras: null,      // deduped inserted words
  extrasSeen: null,  // word -> last-seen ts (stream windows repeat noise)
};
...
function _v2CountError(flashExpected) {
  _v2.errors++;
...
// ── TasmeeMatcher integration (v3) ───────────────────────────────────
// Tokens are built 1:1 from the rendered rect-words, so matcher indices map
// straight back to page spans. Note: on the rare pages where mushaf rects
// don't align 1:1 with text words, a token may hold two words — same
// limitation the v2 inline matcher had.
function _v2BuildTokens() {
  _v2.allTokens = null;
  if (typeof TasmeeMatcher === 'undefined') { _v2.matcher = null; return; }
  const m = new TasmeeMatcher();
  const toks = _v2.words.map((w, i) => ({
    idx: i, surah: w.surah, ayah: w.ayah, wordIdxInAyah: w.word,
    raw: w.span.textContent, norm: m.normArabic(w.span.textContent),
    state: 'pending', variants: null
  }));
  for (let i = 0; i < toks.length; i++) {
    toks[i].variants = m.variantsFor(toks[i].raw, toks[i].norm, toks[i + 1] ? toks[i + 1].norm : '');
  }
  _v2.allTokens = toks;
  _v2SyncMatcher();
}

// Rebuild the matcher window from the current pointer. Called after every
// MANUAL pointer move (medallion start, tpPrev, tpSkipBack, verse-ID jump) —
// during streaming the matcher itself advances and _v2.ptr follows it.
function _v2SyncMatcher() {
  if (!_v2.allTokens) return;
  if (!_v2.matcher) {
    _v2.matcher = new TasmeeMatcher({ onWord: _v2OnMatcherWord, onExtra: _v2OnMatcherExtra });
  }
  _v2.tokenOffset = _v2.ptr;
  const slice = _v2.allTokens.slice(_v2.ptr).map((t, i) =>
    Object.assign({}, t, { idx: i, state: 'pending' }));
  _v2.matcher.setTokens(slice);
}

function _v2OnMatcherWord(t, state) {
  const gi = _v2.tokenOffset + t.idx;
  if (state === 'correct' || state === 'fuzzy') {
    _v2RevealWord(gi, false);
    _v2Record(t, state);
  } else if (state === 'missed') {
    _v2RevealWord(gi, true);
    _v2Record(t, 'missed');
    _v2CountError(false);   // badge only — the missed word is already styled
  }
}

// ASR noise repeats across overlapping stream windows — record an inserted
// ("زائد") word at most once per short interval.
function _v2OnMatcherExtra(word) {
  if (!_v2.extras) return;
  const now = Date.now();
  if (!_v2.extrasSeen) _v2.extrasSeen = new Map();
  if (now - (_v2.extrasSeen.get(word) || 0) < 6000) return;
  _v2.extrasSeen.set(word, now);
  _v2.extras.push(word);
}

// Last-wins per-word record: retrying an ayah (tpSkipBack) overwrites the
// earlier attempt instead of double-counting it.
function _v2Record(t, state) {
  if (!_v2.results) return;
  _v2.results.set(t.surah + ':' + t.ayah + ':' + t.wordIdxInAyah,
    { surah: t.surah, ayah: t.ayah, word: t.raw, state });
}

// Persist session + typed mistakes. v2 kept only an in-memory error counter,
// so the dashboard never saw Pro-panel sessions at all.
function _v2SaveSession() {
  try {
    if (typeof TasmeeStore === 'undefined' || !_v2.results || _v2.results.size === 0) return;
    const vals = Array.from(_v2.results.values());
    let correct = 0, fuzzy = 0, missed = 0;
    vals.forEach(r => {
      if (r.state === 'correct') correct++;
      else if (r.state === 'fuzzy') fuzzy++;
      else if (r.state === 'missed') missed++;
    });
    const total = vals.length;
    if (!total || (correct + fuzzy) === 0) return;   // nothing actually recited
    let fromAyah = Infinity, toAyah = 0;
    vals.forEach(r => { if (r.ayah < fromAyah) fromAyah = r.ayah; if (r.ayah > toAyah) toAyah = r.ayah; });
    TasmeeStore.addSession({
      surah: vals[0].surah, fromAyah, toAyah, page: _v2.page,
      correct, fuzzy, missed, total,
      accuracy: ((correct + fuzzy) / total) * 100,
      durationSec: _v2.sessionStart ? Math.round((Date.now() - _v2.sessionStart) / 1000) : 0,
      mode: 'tasmee-pro'
    }).catch(() => {});
    const mistakes = vals
      .filter(r => r.state === 'missed' || r.state === 'fuzzy')
      .map(r => ({ surah: r.surah, ayah: r.ayah, word: r.word,
                   type: r.state === 'missed' ? 'missing' : 'wrong' }));
    (_v2.extras || []).forEach(w => mistakes.push({ surah: null, ayah: null, word: w, type: 'extra' }));
    if (mistakes.length) TasmeeStore.addMistakes(mistakes).catch(() => {});
  } catch (e) { /* persistence must never break the panel */ }
  _v2.results = new Map();
  _v2.extras = [];
}
...
function _tpHandleStreamResult(data) {
  ...
  if (_v2.matcher && _v2.allTokens) {
    // Shared brain: re-anchors inside the overlapping window, advances
    // forward-only; reveals/misses arrive via the _v2OnMatcherWord callback.
    _v2.matcher.feedTranscript(data.transcript);
    const newPtr = _v2.tokenOffset + _v2.matcher.position;
    if (newPtr > _v2.ptr) { _v2.ptr = newPtr; _v2MarkExpected(); }
    return;
  }

  // Fallback (shared matcher failed to load) — original v2 inline matching.
...
function _aiHandleResult(result) {
  ...
  // The whole ayah was recognized as recited — record its words as correct
  // so the persisted session reflects the verse-ID path too.
  if (_v2.allTokens && _v2.results) {
    for (let i = range.start; i <= range.end; i++) {
      const t = _v2.allTokens[i];
      if (t) _v2Record(t, 'correct');
    }
  }
...
async function _v2Open(pageNumber) {
  ...
  _v2SaveSession();            // flush anything left from a previous page
  _v2.page = pageNumber;
  _v2.errors = 0;
  _v2.peek = false;
  _v2.sessionStart = Date.now();
  _v2.results = new Map();
  _v2.extras = [];
  _v2.extrasSeen = new Map();
...
function closeTasmeePro() {
  ...
  _v2.allTokens = null; _v2.tokenOffset = 0;
  const host = document.getElementById('tpv2PageHost');
